	  ...
	  struct samhist **shout)
{
  ...
  double *Z, *buffer;
  double *f1, *f2;
  int no_succ = 0, no_try = 0, last_del_chg = 0, del_estim_len = 0;

  int no_mod = 0, no_ver = 0, no_pass = 0;
  double A = 0, linl, lout, la, prolong = 0, new_del, new_ep;
  int lastAchg = 0, max_sam_updated = 1;
  struct samhist *shistory = NULL;

  double poly[4], roots[3], f[9], *err, *sgn;
  int *inls[5], *iinls[3];
  int nsol, i, j, *inliers, new_max, do_iterate, pos;
  int maxI, maxIs, I, cI, min_iter_inl;
  int *samidx, *d, *curr_inl, *iter_inl, *max_inl = 0;
  int *pool2, ver_st = 0;
  int * bailI = {0};

  double mA[7*9], sol[2*9];
  ...
  while(no_sam < max_sam)
    {
      ...
      rsampleT(Z, 9, pool, 7, len, mA);

      /* QR */
      nullspace_qr7x9(mA, sol);
